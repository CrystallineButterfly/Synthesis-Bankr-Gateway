...
import {ComputeBudgetVault} from "src/ComputeBudgetVault.sol";

contract ComputeBudgetVaultTest is Test {
    ComputeBudgetVault private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_bankr_gateway");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new ComputeBudgetVault(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        ComputeBudgetVault freshController = new ComputeBudgetVault(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
